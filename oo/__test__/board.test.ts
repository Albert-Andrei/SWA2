  ...
  function toEqual(...tiles: String[]) {
    for (let row: number = 0; row < board.height; row++) {
      for (let col: number = 0; col < board.width; col++) {
        expect(board.piece({ row, col })).toEqual(
    ...
    describe('valid moves', () => {
      ...
      describe('invalid moves', () => {
        it('does not allow moves that make no matches', () => {
          expect(board.canMove({ row: 0, col: 0 }, { row: 0, col: 0 })).toEqual(
            false,
          );
        });
        it('does not count the piece that is moved away', () => {
          expect(board.canMove({ row: 1, col: 1 }, { row: 2, col: 1 })).toEqual(
            false,
          );
        });
        it('recognizes moves on different rows and columns as invalid', () => {
          expect(board.canMove({ row: 0, col: 3 }, { row: 1, col: 2 })).toEqual(
            false,
          );
        });
        it('recognizes out-of-bounds moves as invalid', () => {
          expect(
            board.canMove({ row: 3, col: 3 }, { row: -1, col: 3 }),
          ).toEqual(false);
          expect(
            board.canMove({ row: 3, col: 3 }, { row: 3, col: -1 }),
          ).toEqual(false);
          expect(board.canMove({ row: 2, col: 0 }, { row: 2, col: 4 })).toEqual(
            false,
          );
        });
      });
