  ...
  function toEqual(...tiles: String[]) {
    for (let row: number = 0; row < board.height; row++) {
      for (let col: number = 0; col < board.width; col++) {
        expect(Board.piece(board, { row, col })).toEqual(
          tiles[row * board.width + col],
        );
      }
    }
  }
  ...
  describe('moves', () => {
    describe('valid moves', () => {
      ...
      describe('invalid moves', () => {
        it('does not allow moves that make no matches', () => {
          expect(
            Board.canMove(board, { row: 0, col: 0 }, { row: 0, col: 0 }),
          ).toEqual(false);
        });
        it('does not count the piece that is moved away', () => {
          expect(
            Board.canMove(board, { row: 1, col: 1 }, { row: 2, col: 1 }),
          ).toEqual(false);
        });
        it('recognizes moves on different rows and columns as invalid', () => {
          expect(
            Board.canMove(board, { row: 0, col: 3 }, { row: 1, col: 2 }),
          ).toEqual(false);
        });
        it('recognizes out-of-bounds moves as invalid', () => {
          expect(
            Board.canMove(board, { row: 3, col: 3 }, { row: -1, col: 3 }),
          ).toEqual(false);
          expect(
            Board.canMove(board, { row: 3, col: 3 }, { row: 3, col: -1 }),
          ).toEqual(false);
          expect(
            Board.canMove(board, { row: 2, col: 0 }, { row: 2, col: 4 }),
          ).toEqual(false);
        });
      });
    });
    ...
    describe('making moves', () => {
      ...
      it("doesn't fire events on illegal moves", () => {
        generator.prepare('C', 'D', 'A', 'C', 'D', 'A', 'C', 'D', 'A');
        expect(
          Board.move(generator, board, { row: 0, col: 0 }, { row: 0, col: 0 })
            .effects,
        ).toEqual([]);
        expect(
          Board.move(generator, board, { row: 1, col: 1 }, { row: 2, col: 1 })
            .effects,
        ).toEqual([]);
        expect(
          Board.move(generator, board, { row: 0, col: 3 }, { row: 1, col: 2 })
            .effects,
        ).toEqual([]);
        expect(
          Board.move(generator, board, { row: 3, col: 3 }, { row: -1, col: 3 })
            .effects,
        ).toEqual([]);
      });
    });
